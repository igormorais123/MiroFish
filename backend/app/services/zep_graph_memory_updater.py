"""
Servico de atualizacao da memoria em grafo via Graphiti Server.
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Converte a atividade em uma descricao textual adequada para o Graphiti.
        """
        action_descriptions = {
...
class ZepGraphMemoryUpdater:
    """
    Atualizador de memoria em grafo via Graphiti Server.

    Monitora os logs de acoes da simulacao e envia atividades relevantes em
    lote, agrupadas por plataforma, usando POST /messages.
    """
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        Args:
            graph_id: Identificador do grupo no Graphiti (equivale ao graph_id).
            api_key: Mantido para compatibilidade. Nao utilizado pelo Graphiti.
        """
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        logger.debug(f"Atividade adicionada a fila: {activity.agent_name} - {activity.action_type}")
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """Envia um lote de atividades ao Graphiti como mensagens."""
        if not activities:
            return

        # Constroi mensagens a partir das atividades
        messages = []
        for activity in activities:
            messages.append({
                ...
                "source_description": f"Atividade de simulacao ({platform}, rodada {activity.round_num})",
            })
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                logger.info(f"Lote enviado com sucesso: {len(activities)} atividades de {display_name} para o grupo {self.graph_id}")
                return

            except Exception as e:
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"Falha ao enviar lote ao Graphiti (tentativa {attempt + 1}/{self.MAX_RETRIES}): {e}")
                    time.sleep(self.RETRY_DELAY * (attempt + 1))
                else:
                    logger.error(f"Falha ao enviar lote ao Graphiti apos {self.MAX_RETRIES} tentativas: {e}")
