      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-timeline">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <div v-if="log.details?.has_final_answer" class="final-answer-hint">
                      ...
                      <span>Conteúdo da seção "{{ log.section_title }}" gerado</span>
                    </div>
                  ...
                  <template v-if="log.action === 'report_complete'">
                    <div class="complete-banner">
                      ...
                      <span>Geração do relatório concluída</span>
                    </div>
                  </template>
                </div>
                ...
                <div class="timeline-footer" v-if="log.elapsed_seconds || (log.action === 'tool_call' && log.details?.parameters) || log.action === 'tool_result' || (log.action === 'llm_response' && log.details?.response)">
                  ...
                  <div class="footer-actions">
                    ...
                    <button v-if="log.action === 'tool_call' && log.details?.parameters" class="action-btn" @click.stop="toggleLogExpand(log)">
                      {{ expandedLogs.has(log.timestamp) ? 'Ocultar parâmetros' : 'Mostrar parâmetros' }}
                    </button>
                    ...
                    <button v-if="log.action === 'llm_response' && log.details?.response" class="action-btn" @click.stop="toggleLogExpand(log)">
                      {{ expandedLogs.has(log.timestamp) ? 'Ocultar resposta' : 'Mostrar resposta' }}
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div v-if="agentLogs.length === 0 && !isComplete" class="workflow-empty">
            ...
            <span>Aguardando atividade do agente...</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="console-logs">
      ...
      <div class="log-content" ref="logContent">
        <div class="log-line" v-for="(log, idx) in consoleLogs" :key="idx">
          <span class="log-msg" :class="getLogLevelClass(log)" :title="log">{{ log }}</span>
        </div>
      </div>
    </div>
  </div>
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        h('div', { class: 'header-main' }, [
          ...
          h('div', { class: 'header-stats' }, [
            ...
            h('span', { class: 'stat-item' }, [
              ...
              h('span', { class: 'stat-label' }, 'Relações')
            ]),
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Cenário previsto: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memórias-chave atuais (${props.result.facts.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Entidades centrais (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, `Cadeia de relações (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Subquestões (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Fatos-chave mais recentes associados às memórias temporais'),
            h('span', { class: 'panel-count' }, `${props.result.facts.length} no total`)
          ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Entidades centrais'),
            h('span', { class: 'panel-count' }, `${props.result.entities.length} no total`)
          ]),
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount} fatos`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `Recolher ▲` : `Expandir tudo ${props.result.entities.length} ▼`)
        ]),
        
        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Cadeia de relações'),
            h('span', { class: 'panel-count' }, `${props.result.relations.length} no total`)
          ]),
          ...
          }, expandedRelations.value ? `Recolher ▲` : `Expandir tudo ${props.result.relations.length} ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Subquestões geradas pela análise da consulta'),
            h('span', { class: 'panel-count' }, `${props.result.subQueries.length} no total`)
          ]),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'Nenhuma memória-chave atual'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Nenhuma entidade central'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Nenhuma cadeia de relações')
      ])
    ])
  }
}
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-header' }, [
        h('div', { class: 'header-main' }, [
          h('div', { class: 'header-title' }, 'Busca panorâmica'),
          h('div', { class: 'header-stats' }, [
            h('span', { class: 'stat-item' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memórias ativas (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, `Memórias históricas (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Entidades envolvidas (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Memórias ativas'),
            h('span', { class: 'panel-count' }, `${props.result.activeFacts.length} no total`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Nenhuma memória ativa'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `Recolher ▲` : `Expandir tudo ${props.result.activeFacts.length} ▼`)
        ]),
        
        // Historical Facts Tab
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Memórias históricas'),
            h('span', { class: 'panel-count' }, `${props.result.historicalFacts.length} no total`)
          ]),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Tenta extrair a informação de tempo [time - time]
                  (() => {
          ...
          ) : h('div', { class: 'empty-state' }, 'Nenhuma memória histórica'),
          props.result.historicalFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedHistorical.value = !expandedHistorical.value }
          }, expandedHistorical.value ? `Recolher ▲` : `Expandir tudo ${props.result.historicalFacts.length} ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Entidades envolvidas'),
            h('span', { class: 'panel-count' }, `${props.result.entities.length} no total`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Nenhuma entidade envolvida'),
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'agent-tabs' }, 
        props.result.interviews.map((interview, i) => h('button', {
          ...
          h('span', { class: 'tab-name' }, interview.title || interview.name || `Agente ${i + 1}`)
        ]))
      ),
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        h('div', { class: 'agent-profile' }, [
          ...
          h('div', { class: 'profile-name' }, props.result.interviews[activeIndex.value]?.name || 'Agente'),
        ...
        props.result.interviews[activeIndex.value]?.quotes?.length > 0 && h('div', { class: 'quotes-section' }, [
          h('div', { class: 'quotes-header' }, 'Citações principais'),
      ...
      props.result.summary && h('div', { class: 'summary-section' }, [
        h('div', { class: 'summary-header' }, 'Resumo das entrevistas'),
        h('div', { 
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: 'quicksearch-header' }, [
        ...
        props.result.query && h('div', { class: 'header-query' }, [
          h('span', { class: 'query-label' }, 'Busca: '),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Nenhum resultado relacionado encontrado'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Recolher ▲` : `Expandir tudo ${props.result.facts.length} ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Relações relacionadas'),
            h('span', { class: 'panel-count' }, `${props.result.edges.length} no total`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Nós relacionados'),
            h('span', { class: 'panel-count' }, `${props.result.nodes.length} no total`)
          ]),
...
const getActionLabel = (action) => {
  const labels = {
    ...
    'planning_complete': 'Plano concluído',
...
onMounted(() => {
  if (props.reportId) {
    addLog(`Agente de relatório inicializado: ${props.reportId}`)
    startPolling()
  }
})
