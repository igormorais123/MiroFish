<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
      <!-- Step 01: Instância da simulação -->
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <div v-if="simulationId" class="info-card">
            ...
            <div class="info-row">
              <span class="info-label">ID do Grafo</span>
              <span class="info-value mono">{{ projectData?.graph_id }}</span>
            </div>
            <div class="info-row">
              <span class="info-label">ID da Simulação</span>
              <span class="info-value mono">{{ simulationId }}</span>
            </div>
            <div class="info-row">
              <span class="info-label">ID da Tarefa</span>
              <span class="info-value mono">{{ taskId || 'Tarefa assíncrona concluída' }}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card-content">
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Tópicos associados aos dados de origem</span>
            </div>
          </div>
            ...
            <div class="profiles-list">
              ...
              >
                <div class="profile-header">
                  <span class="profile-realname">{{ profile.username || 'Desconhecido' }}</span>
                  <span class="profile-username">@{{ profile.name || `agent_${idx}` }}</span>
                </div>
                <div class="profile-meta">
                  <span class="profile-profession">{{ profile.profession || 'Profissão desconhecida' }}</span>
                </div>
                <p class="profile-bio">{{ profile.bio || 'Sem biografia' }}</p>
      ...
      <!-- Step 03: Configuração da simulação nas duas plataformas -->
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  <span class="config-item-label">Duração por rodada</span>
              ...
              <div class="time-periods">
                ...
                <div class="period-item">
                  <span class="period-label">Horário de trabalho</span>
                  <span class="period-hours">{{ simulationConfig.time_config?.work_hours?.[0] }}:00-{{ simulationConfig.time_config?.work_hours?.slice(-1)[0] }}:00</span>
                  <span class="period-multiplier">×{{ simulationConfig.time_config?.work_activity_multiplier }}</span>
                </div>
                <div class="period-item">
                  <span class="period-label">Manhã</span>
                  <span class="period-hours">{{ simulationConfig.time_config?.morning_hours?.[0] }}:00-{{ simulationConfig.time_config?.morning_hours?.slice(-1)[0] }}:00</span>
                  <span class="period-multiplier">×{{ simulationConfig.time_config?.morning_activity_multiplier }}</span>
                </div>
                <div class="period-item">
                  <span class="period-label">Baixa atividade</span>
              ...
              <div class="agents-cards">
                ...
                >
                  ...
                  <div class="agent-timeline">
                    <span class="timeline-label">Faixa de atividade</span>
                  ...
                  <div class="agent-params">
                    <div class="param-group">
                      ...
                      <div class="param-item">
                        <span class="param-label">Comentários/h</span>
                        <span class="param-value">{{ agent.comments_per_hour }}</span>
                      </div>
                      <div class="param-item">
                        <span class="param-label">Atraso de resposta</span>
                        <span class="param-value">{{ agent.response_delay_min }}-{{ agent.response_delay_max }}min</span>
                      </div>
                    </div>
                    <div class="param-group">
                      <div class="param-item">
                        <span class="param-label">Atividade</span>
                      ...
                      <div class="param-item">
                        <span class="param-label">Viés emocional</span>
                        <span class="param-value" :class="agent.sentiment_bias > 0 ? 'positive' : agent.sentiment_bias < 0 ? 'negative' : 'neutral'">
                          {{ agent.sentiment_bias > 0 ? '+' : '' }}{{ agent.sentiment_bias?.toFixed(1) }}
                        </span>
                      </div>
                      <div class="param-item">
                        <span class="param-label">Influência</span>
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Configuração do algoritmo de recomendação</span>
              </div>
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">Plataforma 1: feed aberto</span>
                  </div>
                  <div class="platform-params">
                    ...
                    <div class="param-row">
                      <span class="param-label">Intensidade da bolha</span>
                ...
                <div v-if="simulationConfig.reddit_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">Plataforma 2: comunidade</span>
                  </div>
                  <div class="platform-params">
                    <div class="param-row">
                      <span class="param-label">Peso de recência</span>
      ...
      <!-- Step 04: Orquestração inicial -->
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">04</span>
            <span class="step-title">Orquestração inicial</span>
          </div>
          <div class="step-status">
      ...
      <!-- Step 05: Preparação concluída -->
      <div class="step-card" :class="{ 'active': phase === 4 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">05</span>
            <span class="step-title">Preparação concluída</span>
          </div>
          <div class="step-status">
            <span v-if="phase >= 4" class="badge processing">Disponível</span>
            <span v-else class="badge pending">Aguardando</span>
          </div>
        </div>
        ...
        <div class="card-content">
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">A camada de simulação da INTEIA planejou <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> horas de mundo real, e cada rodada representa <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutos.</span>
              </div>
              <label class="switch-control">
                <input type="checkbox" v-model="useCustomRounds">
                <span class="switch-track"></span>
                <span class="switch-label">Personalizar</span>
              </label>
            </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">Se esta for a primeira execução, use o modo personalizado para reduzir as rodadas, acelerar a prévia e diminuir o risco de falhas ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="action-group dual">
            <button 
              ...
              @click="$emit('go-back')"
            >
              ← Voltar para a construção do grafo
            </button>
            <button 
            ...
            >
              Iniciar simulação paralela dos dois mundos ➝
            </button>
          </div>
        </div>
      </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">Idade aparente</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} anos</span>
            </div>
            <div class="info-item">
              <span class="info-label">Gênero aparente</span>
            ...
            <div class="info-item">
              <span class="info-label">MBTI aparente</span>
          ...
          <div class="modal-section">
            <span class="section-label">Resumo do perfil</span>
            <p class="section-bio">{{ selectedProfile.bio || 'Sem biografia' }}</p>
          </div>

          <!-- Topicos de interesse -->
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">Tópicos ligados aos dados de origem</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            <span class="section-label">Contexto detalhado do perfil</span>
            
            <!-- Visao geral das dimensoes do perfil -->
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">Experiência no evento</span>
                <span class="dim-desc">Trajetória completa dentro do cenário</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Padrão de comportamento</span>
                <span class="dim-desc">Síntese de hábitos e preferências de ação</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Memórias marcantes</span>
                <span class="dim-desc">Memórias formadas a partir dos dados reais</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Rede social</span>
                <span class="dim-desc">Conexões individuais e mapa de interação</span>
              </div>
            </div>
...
// State
const phase = ref(0) // 0: inicialização, 1: perfis, 2: configuração, 3: concluído
...
// Deduplicação de logs: registrar última informação relevante
...
watch(currentStage, (newStage) => {
  if (newStage === 'Geração de perfis de agentes' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === 'Geração da configuração da simulação' || newStage === 'generating_config') {
    phase.value = 2
    // Entrar na fase de geração de configuração, iniciar polling da configuração
    if (!configTimer) {
      addLog('Iniciando a geração da configuração da simulação em duas plataformas...')
      startConfigPolling()
    }
  } else if (newStage === 'Preparacao dos scripts da simulação' || newStage === 'copying_scripts') {
    phase.value = 2 // Ainda pertence a fase de configuração
  }
})

// Calcular rodadas automaticas a partir da configuração (sem valores padrão fixos)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Retornar null quando a configuração não foi gerada
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // Retornar null quando os dados da configuração estao incompletos
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // Garantir que o maximo de rodadas não seja menor que 40 (recomendado), evitando anomalias no slider
...
// Obter username correspondente ao agent_id
...
// Calcular total de topicos associados a todos os perfis
...
// Tratar clique no botao de iniciar simulação
const handleStartSimulation = () => {
  // Construir parâmetros para passar ao componente pai
  const params = {}
  
  if (useCustomRounds.value) {
    // Rodadas personalizadas pelo usuário, passar parâmetro max_rounds
    params.maxRounds = customMaxRounds.value
    addLog(`Iniciando a simulação com número de rodadas personalizado: ${customMaxRounds.value}`)
  } else {
    // Usuario escolheu manter rodadas automaticas, não passar parâmetro max_rounds
    addLog(`Iniciando a simulação com o total automático de rodadas: ${autoGeneratedRounds.value}`)
  }
...
// Iniciar preparação da simulação automaticamente
const startPrepareSimulation = async () => {
  ...
  // Marcar primeira etapa como concluida, iniciar segunda etapa
  phase.value = 1
  addLog(`Instância de simulação criada: ${props.simulationId}`)
  addLog('Preparando o ambiente da simulação...')
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Foi detectada uma preparação já concluída. Reutilizando os dados existentes.')
        await loadPreparedData()
        return
      }
      
      taskId.value = res.data.task_id
      addLog('A tarefa de preparação foi iniciada')
      addLog(`  └─ Task ID: ${res.data.task_id}`)
      
      // Definir total esperado de agentes imediatamente (obtido da resposta da API prepare)
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(`Foram lidas ${res.data.expected_entities_count} entidades do grafo da Zep`)
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(`  └─ Tipos de entidade: ${res.data.entity_types.join(', ')}`)
        }
      }
      
      addLog('Iniciando a consulta periódica do progresso da preparação...')
      // Iniciar polling de progresso
      startPolling()
      // Iniciar obtencao em tempo real de Profiles
    ...
    addLog(`Erro durante a preparação: ${err.message}`)
    emit('update-status', 'error')
  }
}
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Analisar informações da fase e emitir logs detalhados
      if (data.progress_detail) {
        currentStage.value = data.progress_detail.current_stage_name || ''
        
        // Emitir logs detalhados de progresso (evitando duplicatas)
        ...
        // Extrair fase da mensagem
        const match = data.message.match(/\[(\d+)\/(\d+)\]\s*([^:]+)/)
        if (match) {
          currentStage.value = match[3].trim()
        }
        // Emitir logs da mensagem (evitando duplicatas)
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Atualizar apenas quando a API retorna valores validos, evitando sobrescrever dados existentes
      if (res.data.total_expected) {
        expectedTotal.value = res.data.total_expected
      }
      
      // Extrair tipos de entidade
      const types = new Set()
      ...
      // Emitir logs de progresso de geração de Profiles (apenas quando a quantidade muda)
    ...
    console.warn('Falha ao obter Profiles:', err)
  }
}
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Emitir logs da fase de geração de configuração (evitando duplicatas)
      ...
      // Se a configuração foi gerada
      if (data.config_generated && data.config) {
        simulationConfig.value = data.config
        addLog('✓ Configuração da simulação gerada')
        ...
        if (data.summary) {
          ...
          addLog(`  └─ Configuração das plataformas: Feed aberto ${data.summary.has_twitter_config ? '✓' : '✗'}, Comunidade ${data.summary.has_reddit_config ? '✓' : '✗'}`)
        }
        
        // Exibir detalhes da configuração de tempo
        if (data.config.time_config) {
          const tc = data.config.time_config
          addLog(`Configuração de tempo: ${tc.minutes_per_round} minutos por rodada, total de ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} rodadas`)
        }
        
        // Exibir configuração de eventos
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Configuração do ambiente concluída. A simulação pode começar.')
        emit('update-status', 'completed')
      }
    }
  } catch (err) {
    console.warn('Falha ao obter Config:', err)
  }
}

const loadPreparedData = async () => {
  phase.value = 2
  addLog('Carregando a configuração já existente...')
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('✓ Configuração da simulação carregada com sucesso')
        ...
        addLog('A configuração ainda está sendo gerada. Iniciando a espera com polling...')
        startConfigPolling()
      }
    }
  } catch (err) {
    addLog(`Falha ao carregar a configuração: ${err.message}`)
    emit('update-status', 'error')
  }
}
...
onMounted(() => {
  // Iniciar fluxo de preparação automaticamente
  if (props.simulationId) {
    addLog('Step2 Inicialização da configuração do ambiente')
...
.step-card.active {
  ...
  box-shadow: 0 20px 40px rgba(212, 160, 23, 0.14);
}
...
.badge.success { background: rgba(94, 122, 52, 0.12); color: #4d6b2d; }
.badge.processing { background: linear-gradient(135deg, #d4a017 0%, #f2c14a 100%); color: #0f2747; }
.badge.pending { background: rgba(15, 39, 71, 0.06); color: #6b7280; }
.badge.accent { background: rgba(15, 39, 71, 0.08); color: #0f2747; }
...
/* Estilo de configuração de rodadas */
...
.range-mark-button:hover,
.range-mark-button.active {
  color: #000;
  font-weight: 600;
}
