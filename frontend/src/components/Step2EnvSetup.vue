          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">O MiroFish planejou <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> horas de mundo real, e cada rodada representa <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutos.</span>
              </div>
...
// State
const phase = ref(0) // 0: inicializacao, 1: perfis, 2: configuracao, 3: concluido
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent人设' || newStage === 'Geracao de perfis de agentes' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === '生成模拟配置' || newStage === 'Geracao da configuracao da simulacao' || newStage === 'generating_config') {
    phase.value = 2
    // 进入配置生成阶段，开始轮询配置
    if (!configTimer) {
  ...
  } else if (newStage === '准备模拟脚本' || newStage === 'Preparacao dos scripts da simulacao' || newStage === 'copying_scripts') {
